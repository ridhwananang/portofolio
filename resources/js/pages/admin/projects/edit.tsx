...
export default function ProjectEdit({ project }: ProjectEditProps) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        mockup_type: project.mockup_type || 'macbook',
        image: null as File | null,
    });

    const [previewImageUrl, setPreviewImageUrl] = useState<string | null>(project.image_url || null);
    ...
    const handleImageChange = (file: File | null) => {
        ...
        if (file) {
            ...
            setPreviewImageUrl(project.image_url || null);
        }
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post(`/admin/projects/${project.id}`, {
            ...
            onError: () => {
                toast.error('Gagal memperbarui proyek. Silakan periksa formulir.');
            },
        });
    };
    ...
    return (
        <AppLayout
            breadcrumbs={[
                ...
                { title: 'Edit Proyek', href: `/admin/projects/${project.id}/edit` },
            ]}
        >
            <Head title={`Edit Proyek: ${project.title} - Ridhwan Studio`} />
                ...
                <div className="flex items-center gap-3.5">
                    ...
                    <div>
                        ...
                        <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                            Perbarui informasi karya, teknologi yang digunakan, atau ubah tampilan mockup
                        </p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-7 items-start">
                    ...
                    <form onSubmit={handleSubmit} className="lg:col-span-7 space-y-6">
                        <div className="glass-card rounded-[2.2rem] border border-slate-200/70 bg-white/75 p-7 sm:p-8 backdrop-blur-xl shadow-xl shadow-slate-100/50 dark:border-slate-800/80 dark:bg-slate-900/60 dark:shadow-none space-y-6">
                            ...
                            <div className="space-y-2">
                                <Label className="text-xs font-bold text-slate-700 dark:text-slate-300">
                                    Ganti Screenshot / Mockup Utama
                                </Label>
                                <ImageUpload
                                    currentImageUrl={project.image_url}
                                    onChange={handleImageChange}
                                    error={errors.image}
                                    label="Upload Screenshot Baru"
                                />
                            </div>
                        </div>
                        ...
                        <div className="glass-card rounded-[2.2rem] border border-slate-200/70 bg-white/75 p-6 backdrop-blur-xl shadow-xl shadow-slate-100/50 dark:border-slate-800/80 dark:bg-slate-900/60 dark:shadow-none space-y-5">
                                    ...
                                    {previewImageUrl ? (
                                        ...
                                        <div className="text-center p-6 space-y-2">
                                            ...
                                            <p className="text-xs text-slate-500">
                                                Pilih gambar untuk melihat mockup hidup di kanvas ini.
                                            </p>
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="space-y-2.5">
                                ...
                                <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
                                    {data.description || 'Deskripsi proyek akan ditampilkan di sini...'}
                                </p>
                            </div>
                        </div>
                    </div>
